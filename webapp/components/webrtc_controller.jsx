...
import * as WebrtcActions from 'actions/webrtc_actions.js';
...
export default class WebrtcController extends React.Component {
    constructor(props) {
        ...
        const {Utils} = props;
        const {ChannelStore, UserStore} = props.stores;
    ...
    componentDidMount() {
        const {UserStore, WebrtcStore} = this.props.stores;
    ...
    handleMakeOffer() {
        ...
        const {UserStatuses, WebrtcActionTypes} = this.props.Constants;
    ...
    handleCancelOffer() {
        const {WebSocketClient} = this.props;
        const {WebrtcActionTypes} = this.props.Constants;
    ...
    close() {
        const {AppDispatcher} = this.props;
        const {WebrtcActionTypes} = this.props.Constants;
        ...
        WebrtcActions.initWebrtc(null, false)(AppDispatcher, WebrtcActionTypes.INITIALIZE);
    }

    onStatusChange() {
        const {UserStore} = this.props.stores;
        const {Utils} = this.props;
        const {UserStatuses} = this.props.Constants;
        ...
        WebrtcActions.webrtcToken(
            (info) => {
                ...
                if (this.session) {
                    ...
                    const SessionFactory = KWM.KWMInit;
    ...
    doHangup(error, manual) {
        const {WebrtcStore} = this.props.stores;
        const {Utils} = this.props;
    ...
    render() {
        const {UserStore} = this.props.stores;
        const {Constants, Utils} = this.props;
                ...
                <div className='sidebar-right__body'>
                    ...
                    <div className='post-right__scroll'>
                        <div id='videos'>
                            ...
                            >
                                <video
                                    ref={video=>this.mainVideo=video}
                                    autoPlay={true}
                                />
                            </div>
                            <div
