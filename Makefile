...
dist: webapp/workspaces/kopanowebmeetings/build/kopanowebmeetings_bundle.js plugin.json ; $(info building dist tarball ...)
	...
	mv -vf "${PACKAGE_NAME}-${VERSION}/webapp/kopanowebmeetings_bundle.js" "${PACKAGE_NAME}-${VERSION}/webapp/kopanowebmeetings_bundle-${VHASH}.js"  && \
	mv -vf "${PACKAGE_NAME}-${VERSION}/webapp/kopanowebmeetings_bundle.js.map" "${PACKAGE_NAME}-${VERSION}/webapp/kopanowebmeetings_bundle-${VHASH}.js.map"  && \
	cp -avr ../plugin.json "${PACKAGE_NAME}-${VERSION}" && \
	sed -i s/0.0.0-no-proper-build/${VERSION}/g "${PACKAGE_NAME}-${VERSION}/plugin.json" && \
	sed -i s/kopanowebmeetings_bundle.js/kopanowebmeetings_bundle-${VHASH}.js/g "${PACKAGE_NAME}-${VERSION}/plugin.json" && \
